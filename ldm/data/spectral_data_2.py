...
class SPECBase(Dataset):
    ...
    """
    def __init__(self, data_root, size=128, interpolation="nearest", mode=None, num_classes=1):
    ...
    def __getitem__(self, i):
        ...
        path2 = example["file_path_"].replace('syntrainpng','syntrainclue2d')
        path2 = path2.replace('png','mat')
        clue = sio.loadmat(path2)
        ...
        #if self.mode == "train":
        image = self._utilize_transformation(image, self.transform)
        ...
        tempclue = self.transform(torch.tensor(clue['clue2d'][:,:,0:8].astype(np.float32).transpose((2,0,1))))
        example["specmap"] = tempclue.numpy()
    ...
    def _parse_data_list(self): # 80% / 10% / 10%
        ...
        train_imgs, val_imgs, test_imgs = all_imgs[:4000], all_imgs[4000:4200], all_imgs[4200:4300]
        ...
        if self.mode == "train":
            ...
            test_imgs = glob.glob(os.path.join('', "*.png"))
            return test_imgs
        else:
            raise NotImplementedError(f"Only support dataset split: train, val, test !")

    @staticmethod
    def _utilize_transformation(image, func):
...
class SPECTrain(SPECBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="", mode="train")#, **kwargs


class SPECValidation(SPECBase):
    def __init__(self, **kwargs):
        super().__init__(data_root="", mode="val")


class SPECTest(SPECBase):
    def __init__(self, **kwargs):
